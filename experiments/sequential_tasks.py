...
def print_results(baseline_ppls, lora_matrix, nplm_matrix):
    n   = len(TASK_NAMES)
    col = 10
    W   = 72
    ...
    print(f"{'─'*W}")
    print(f"  {'':30s}  {'Sci PPL':>{col}}  {'Δ vs T1':>{col}}")
    print(f"  {'─'*30}  {'─'*col}  {'─'*col}")
    print(f"  {'Baseline (frozen base)':30s}  {baseline_ppls['science']:>{col}.2f}  {'—':>{col}}")
    ...
    for i, task in enumerate(TASK_NAMES):
        ...
        print(f"\n  After {tasks_str}")
        print(f"    {'Sequential LoRA':28s}  {lora_ppl:>{col}.2f}  {lora_d:>{col}}")
        print(f"    {'NeuroplasticLFM':28s}  {nplm_ppl:>{col}.2f}  {nplm_d:>{col}}")
    ...
    print(f"  Sequential LoRA:  {lora_forget:+.3f}  ({lora_forget/lora_t1*100:+.1f}%)")
    print(f"  NeuroplasticLFM:  {nplm_forget:+.3f}  ({nplm_forget/nplm_t1*100:+.1f}%)")
    print(f"{'═'*W}")
    ...
    print(f"{'─'*W}")
    _matrix_block("SEQUENTIAL LORA", lora_matrix)
    _matrix_block("NEUROPLASTICLFM", nplm_matrix)
    print(f"{'═'*W}")
...
def main():
    ...
    # ── Experiment 1: Sequential LoRA ─────────────────────────────────────────
    print(f"\n{'═'*60}")
    print("EXPERIMENT 1: Sequential LoRA (one shared adapter)")
    print(f"{'═'*60}")
    ...
    # ── Experiment 2: NeuroplasticLFM ─────────────────────────────────────────
    print(f"\n{'═'*60}")
    print("EXPERIMENT 2: NeuroplasticLFM (5 frozen CfC clusters)")
    print(f"{'═'*60}")
    nplm_matrix = run_neuroplastic(base, train_dls, eval_dls, device)

    print_results(baseline_ppls, lora_matrix, nplm_matrix)
